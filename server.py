...
if syminfo.ticker == "{sym}"
    ...
    if showHVL
        ...
        if not na(hvl_y)
            ...
            h_lab := label.new(bar_index + 15, hvl_y, " منطقة شهرية متوقعه  " + str.tostring(hvl_y, "#.##") + (colHVL == color.lime ? "  (🟢)" : colHVL == color.red ? "  (🔴)" : "  (🟡)") + " ±" + str.tostring(zoneWidth, "#.##") + "%", style = label.style_label_left, textcolor = color.black, color = colHVL, size = size.small)
